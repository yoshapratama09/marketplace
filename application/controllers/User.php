...
class User extends CI_Controller {
	...
	public function register()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success alert">Registrasi Berhasil</div>');
			redirect('user/login');
		}
			else
			{
			...
			else
			{
				...
				if ($this->ion_auth->login_front($this->input->post('username'), $this->input->post('password'), $remember))
				{
					...
					else
					{
						...
						$this->session->set_flashdata('message', '<div class="col-lg-12"><div class="alert alert-danger">Gagal login</div></div>');
